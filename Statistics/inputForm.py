...
class KendallWValueForms(forms.Form):
    set_1 = setField(label='Set 1', max_length=100, help_text="1,2,3", required=True)
    set_2 = setField(label='Set 2', max_length=100, help_text="3,4,5", required=True)

    # def clean_set_1(self):
    #     # raise an exception if the values are not csv
    #     value = self.cleaned_data.get('set_1')
    #     # raise forms.ValidationError("VFUCKJKDLFJL")
    #     err = "someting"
    #     self.add_error("set_1", err)
